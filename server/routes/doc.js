...
module.exports = (app) => {
  ...
  app.get('/api/getcode', verifyController.getCode);
  app.get('/api/verifycode', verifyController.verifyCode);
          ...
          return new Promise((resolve, reject) => {
            v.drag_data.map((drag) => {
              if (!ids[req.body.id].includes(drag.signer_id)) {
                ...
                reject('').catch((err) => next(err));
              }
            });
          });
        };
        Promise.all(doc.images.map(fn)).catch((err) => next(err));
  ...
  app.delete('/api/que/:queid/:docid', async(req,res,next) => {
    ...
    app.put('/api/doc/:id', async (req, res, next) => {
      ...
      console.log("Signer save ===>" + req.body);
