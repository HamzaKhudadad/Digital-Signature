...
import {Layout, Menu, Divider, Row, Col, Typography, Dropdown,PageHeader, Tag, Spin, Card, Statistic, Button, Table, Progress} from 'antd';
import {
  MenuUnfoldOutlined,
  MenuFoldOutlined,
  UserOutlined,
...
class Dashboard extends Component {
  ...
  appendId = (e) => {
    ...
    console.log(dragData.flat());
    const newdata = dragData.flat()
    Object.keys(newdata).forEach(function (key) {
      if(newdata[key].type == "signer_added"){
        ids.push(newdata[key].signer_id);
      }
    });
  ...
  sendEmail(e){
    ...
    if(uniqueemails.length > 0 && id){
      axios.post('/api/sendemail',{'emails':uniqueemails,'subject':this.state.subject,'message':this.state.message,'id':id,token:localStorage.getItem('jwtToken')}).then((res) => {
        ...
        console.log("-------------hjbjhhgvghvghvghvghvghvghvgh--------------",res)
        this.getSignersWithDoc(res.data.ids);
        swal("Success!", "Document Shared Successfully", "success");
      }).catch(error => {
        swal("Error!", error.response && error.response.data.error || 'Something went wrong', "error");
      });
    }
    return false;
  }
          ...
          <Layout className="site-layout" style={{minHeight: '100vh'}}>
            <Header className="site-layout-background" style={{padding: 0, background:'#f8a740'}}>
             ...
             <PageHeader
                ...
                extra={[
                      <Button className='custom' ><EditTwoTone twoToneColor="#fead01"/>  Update Contact</Button>,
                      <Button className='custom'  disabled={this.state.logoutloading}> 
                          <LogoutOutlined style={{ color:'#fead01'}}/>
                        Logout
                      <Spin spinning={this.state.logoutloading} indicator={antIcon}/>
                      </Button>,
                    ]}
              ...
              <Row justify="space-around" >
                <Content className = "site-layout-background" style = {{margin: '20px 16px 0px 16px', padding: '10px', background:'white'}}>
                  <Col style={{textAlign:'center'}}>
                    <Text style={{fontSize:'12pt',fontWeight:'bold'}}>Signed :</Text> 2
                  </Col>
                </Content>
                <Content className = "site-layout-background" style = {{margin: '20px 16px 0px 16px', padding: '10px', background:'white'}}>
                  <Col style={{textAlign:'center'}}>
                    <Text style={{fontSize:'12pt', fontWeight:'bold'}}>Awaiting Signature : </Text> 2
                  </Col>
                </Content>
                <Content className = "site-layout-background" style = {{margin: '20px 16px 0px 16px', padding: '10px', background:'white'}}>
                  <Col style={{textAlign:'center'}}>
                    <Text style={{fontSize:'12pt', fontWeight:'bold'}}>Uploaded : </Text> 4
                  </Col>
                </Content>
             </Row>
             <Row>
                  ...
                  <Menu
                    ...
                    <Menu.Item key = "Documents" icon = { <MailOutlined/>} >My Documents </Menu.Item>
                    <Menu.Item key="SignedDocuments" icon={<MailOutlined/>}>Signed</Menu.Item>
                    <Menu.Item key="AwaitingSignatures" icon={<AppstoreOutlined/>}>Awaiting Signatures</Menu.Item>
                    <Menu.Item key="Upload" icon={<AppstoreOutlined/>}>Upload</Menu.Item>
                    <a style={{color: '#fead01'}} > back</a>
                  </Menu>
                  {this.state.MenuItem === 'Documents' ? <Documents docs={this.state.docs} appendId={this.appendId} deleteDoc={this.deleteDoc}/> : null}
                  {this.state.MenuItem === 'Upload' ? <UploadDocument docUpload={this.docUpload}/> : null}
                  </Content>
                ...
                <div className="modal-content">
                  ...
                  {addedAlert}
                      ...
                      <tbody>
                          ...
                          <tr>
                            <td><span className=""><button id="cancelButton" data-dismiss="modal" style={{width: '24%',float: 'left'}} className="btn btn-danger cancelButton" type="button"><span className="ui-button-text ui-c">Cancel</span></button>
                            <button id="inviteButton" style={{width: '24%',float: 'right'}} name="inviteButton" className="btn btn-success" type="submit">
                              <span className="">Send Document</span>
                              </button></span></td>
                          </tr>
                      </tbody>
                    </table>
